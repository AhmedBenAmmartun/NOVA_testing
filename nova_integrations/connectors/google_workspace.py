...
class GoogleWorkspaceConnector(WorkspaceConnector):
        ...
        *,
        ...
        try:
            ...
            raise ConnectorError(
                "Google integration packages are missing. "
                "Install requirements-email-calendar.txt."
            ) from exc

        client_path = Path(
            self.config.google.client_config_path
        ).expanduser()

        if not client_path.is_file():
            raise ConnectorError(
                "Google OAuth Desktop client JSON was not found. "
                "Put its local path in the NOVA integrations config."
            )
        ...
        if "mail" in services:
            scopes.append(
                GMAIL_READONLY
                if content_mode != ContentMode.METADATA_ONLY
                else GMAIL_METADATA
            )
        ...
        credentials = flow.run_local_server(
            ...
            authorization_prompt_message=(
                "NOVA is opening Google authorization in your browser."
            ),
            success_message=(
                "NOVA Google authorization completed. "
        ...
        profile = self._userinfo(access_token)
        ...
        if not subject:
            raise ConnectorError(
                "Google did not return a stable account identity."
            )
    ...
    def test_account(
        ...
        return (
            f"Google account '{account.label}' "
            f"is connected for "
            f"{', '.join(enabled) or 'no services'}."
        )
    ...
    def sync_mail(
        ...
        if not history_id:
            initial_items = self._gmail_initial(
                gmail,
                account,
            )
    ...
    def sync_calendar(
        ...
        time_min = (
            now
            - timedelta(
                days=self.config.sync.calendar_past_days
            )
        ).isoformat()
        ...
        time_max = (
            now
            + timedelta(
                days=self.config.sync.calendar_future_days
            )
        ).isoformat()
    ...
    def _gmail_initial(
        ...
        response = (
            ...
            .list(
                ...
                labelIds=["INBOX"],
                maxResults=(
                    self.config.sync.initial_mail_limit
                ),
            )
            .execute()
        )
    ...
    def _google_event(
        ...
        original = (
            raw.get("originalStartTime")
            or raw.get("start")
            or {}
        )

        occurrence_id = str(
            original.get("dateTime")
    ...
    def _raise_google(
        ...
        if status in {401, 403}:
            raise ReconnectRequired(
                ...
                "or lacks the requested read-only scope."
            ) from exc
        ...
        raise ConnectorError(
            "Google synchronization failed without "
            "exposing private response content."
        ) from exc
