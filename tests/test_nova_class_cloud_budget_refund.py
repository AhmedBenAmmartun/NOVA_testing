...
def test_a_dead_provider_no_longer_drains_the_shared_class_cap(
    ...
    safe because a failure now costs nothing -- otherwise the dead provider
    spends half of every question's budget and takes the healthy one down
    with it.
    """
    ...
    assert len(providers["openai"].calls) == 10, "openai was attempted every time"
