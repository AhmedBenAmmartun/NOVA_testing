...
| `nova-file-inventory.txt` (348 KB, repo root) | Untracked, gitignored, not referenced by any script or doc found. A one-time inventory dump from an earlier Codex integration pass. | Safe to delete directly — it's a generated snapshot, not source. |
...
| `NOVA-Email-Calendar-Integration-for-Claude.zip` (59 KB) | Looks like the source bundle that seeded the `nova_integrations/` work for this Claude session. Low ongoing value once that work is committed and reviewed, but cheap to keep a copy short-term. | Same as above. |
| `.nova-patch-backup/` (5 timestamped snapshots, 2026-07-21) | Evidence of iterative in-place patch-and-restore cycles on `Dashboard/` during the desktop-integration work. Gitignored, not tracked. Useful for forensic diffing if something in the current `Dashboard/` regressed silently during those patches. | Keep locally until the current staged `Dashboard/` changes are committed and manually smoke-tested per `CLAUDE.md`'s run-ai-agent driver; archive or delete after. |
...
|---|---|---|
| `core/` (task.py, router.py, orchestrator.py) | `nova_core/` (configuration.py, provider_registry.py, cloud_budget.py, router.py) | `core/` is confirmed dead (imported by nothing, repo-wide grep). `CLAUDE.md`'s own rule says "don't delete it; wire it up or ask Ahmed." Per `FEATURE_AUDIT_AI_TOOLS_PERMISSIONS.md`, `nova_core/` is the newer, more complete design and `core/` adds nothing `nova_core/` lacks. **Recommend deleting `core/` once Ahmed confirms `nova_core/` is the intended long-term router** — but this is an explicit ask-first item per the project's own CLAUDE.md rule, not something to auto-delete. |
...
| `.claude/skills/` | `.agents/skills/` | `PROJECT_STRUCTURE.md` notes `.agents/skills/` "mirrors run-ai-agent plus nova-code-review, nova-tool-pattern — appears to be a Codex-side skills copy." Not independently verified byte-for-byte in this pass. **Needs manual review**: if `.agents/skills/` is Codex's own skill directory (a different tool's config, analogous to `.claude/` for this tool), it is NOT a duplicate to clean up — it's a separate assistant's config living alongside this one. Flagging as unknown rather than guessing. |
...
|---|---|
| `.agents/skills/` vs `.claude/skills/` | Is `.agents/` Codex's own tool-config directory (keep, not a duplicate) or a genuinely stale copy from an earlier setup (candidate for removal)? |
| `CODEX_PROMPTS.md` (21 KB, root) | Per prior session memory, this was adapted from a Downloads-zip Codex kit that targets the *old* Flask Nova, not this repo — still a useful reference doc, or ready to retire now that the real integration work described in it has landed? |
