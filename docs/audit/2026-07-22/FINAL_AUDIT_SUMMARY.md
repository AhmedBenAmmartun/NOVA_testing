...
- **Driver smoke check**: `.claude/skills/run-ai-agent/driver.py tools` → 33/33 (matches CLAUDE.md's last recorded count).
...
## Repository status
- Branch: `codex-nova-desktop-integration`, tracking `origin/codex-nova-desktop-integration`.
- Working tree: dirty (large pre-existing staged change set from an in-progress Codex integration pass, unrelated to this audit — see `GIT_STATE_BEFORE.md`).
...
- **Missing**: Codex app-registry wiring; voice recognition (Stage 6, not started); face recognition (Stage 7, not started).
...
- **Needs manual review**: `.agents/skills/` (possibly Codex's own tool-config, not a duplicate), `CODEX_PROMPTS.md` (possibly retirable now that the work it described has landed), `core/` (confirmed dead but CLAUDE.md's own rule requires asking Ahmed before deleting).
...
- Deleting `core/` (confirmed dead, but CLAUDE.md's own rule requires asking first).
